...
There are many ongoing projects, such as integration of the new dependency resolver pubgrubs, modularizing Cargo, and collaborate with private registry providers to figure out the needs of registry autentications. Some other are exploring ideas on nested workspaces or making Cargo release process more smooth. Also some area like parallsing `libtest` is under consideration. However, `cargo test` depends on `libtest` and may need an edition breaking change for a different mechanism of sharing test environments.
